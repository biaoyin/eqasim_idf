...
public class RunSimulation_BaseCase {
	...
	static public void main(String[] args) throws ConfigurationException {
		...
		config.controler().setOutputDirectory("E:/lvmt_BY/simulation_output/eqasim_idf/ile-de-france-1pm/1pm_test_0");
		config.controler().setOverwriteFileSetting(OutputDirectoryHierarchy.OverwriteFileSetting.deleteDirectoryIfExists);

		//add multi-stage car trips
		config.plansCalcRoute().setAccessEgressType(PlansCalcRouteConfigGroup.AccessEgressType.accessEgressModeToLink);
		config.qsim().setUsingTravelTimeCheckInTeleportation( true );
		//config.qsim().setFlowCapFactor(0.0005);
		//config.qsim().setStorageCapFactor(0.0005);
